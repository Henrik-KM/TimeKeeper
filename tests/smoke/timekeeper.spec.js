...
test('service worker never caches private cross-origin API responses', async () => {
  ...
  expect(serviceWorker).toContain("const CACHE_NAME = 'timekeeper-app-v45';");
  expect(serviceWorker).toContain("'./src/main.mjs?v=40'");
  expect(serviceWorker).toContain(
  ...
  expect(serviceWorker).toContain(
    "url.searchParams.set('timekeeper-update', '45')"
  );
  ...
  expect(mainSource).toContain(".register('./service-worker.js?v=45')");
});
