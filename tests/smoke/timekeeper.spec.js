...
test('service worker never caches private cross-origin API responses', async () => {
  ...
  expect(serviceWorker).toContain("const CACHE_NAME = 'timekeeper-app-v35';");
  expect(serviceWorker).toContain("'./src/main.mjs?v=30'");
  expect(serviceWorker).toContain(
    "'./src/features/codex/performance-worker.mjs'"
  );
  expect(serviceWorker).toContain(
    "url.searchParams.set('timekeeper-update', '32')"
  );
  ...
  expect(mainSource).toContain(".register('./service-worker.js?v=35')");
});
